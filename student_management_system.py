...
=====================================================================
 ...
 them, just with Excel / PDF export features disabled):
...
class DatabaseManager:
    ...
    def __init__(self, db_path=DB_FILE):
        ...
        # check_same_thread=False: Streamlit can serve a request on a
        # different thread than the one that first created the
        # connection, so this is required in this environment.
        self.conn = sqlite3.connect(db_path, check_same_thread=False)
        self.conn.execute("PRAGMA foreign_keys = ON;")
        try:
            self.conn.execute("PRAGMA journal_mode = WAL;")
        except sqlite3.Error:
            pass  # WAL isn't critical; ignore if the filesystem doesn't support it
